...
var MargQAEngine = (function () {
  ...
  function make(topic,k,p){
    ...
    case 5:
      ...
      if(k===3){q='What is the larger root of x² − '+(2*a)+'x + '+(a*a-b*b)+' = 0?';v=a+b;w='Rewrite the quadratic as (x − '+(a-b)+')(x − '+(a+b)+') = 0. The larger root is '+v+'.';insight='Look for a difference-of-squares structure before using the quadratic formula.';}
      if(k===4){q='For real x, what is the minimum value of x² − '+(2*a)+'x + '+(a*a+b)+'?';v=b;w='Complete the square: x² − '+(2*a)+'x + '+(a*a+b)+' = (x − '+a+')² + '+b+'. The square can be zero, so the minimum is '+v+'.';insight='A quadratic minimum can often be read directly after completing the square.';}
      break;
    case 6:
  ...
  function solve(s){var t=topics.indexOf(s.topic),k=s.kind,a=s.p[0],b=s.p[1],c=s.p[2],i,j,n=0;
    ...
    if(t===5){if(k===0){var roots=[];for(i=-40;i<=40;i++)if(i*i-(a+b)*i+a*b===0)roots.push(i);return a===b?2*a*a:roots.reduce(function(x,r){return x+r*r;},0);}if(k===1)return Math.pow(2*a,2)/4;if(k===2){for(i=-40;i<=40;i++)if(i*i-(2*a+b)*i+a*(a+b)<0)n++;return n;}if(k===3)return a+b;if(k===4){var minimum=Infinity;for(i=-50;i<=50;i++){var value=i*i-2*a*i+a*a+b;if(value<minimum)minimum=value;}return minimum;}}
